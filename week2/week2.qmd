...
class(world_df) # solely a data frame now that we dropped geom 

ncol(world_df) # original had 11 columns 
```
...
```{r}
london_proj <- data.frame(x = 530000, y = 1800000) %>% 
...
```{r}
world_pop_density <- world %>% 
  group_by(continent) %>% 
  summarize(pop_density = sum(pop/area_km2, na.rm = TRUE)) %>% 
