...
```{r include=TRUE}
# Crop landsat scene to the extent of the Santa Barbara County shapefile 
...
```{r}
# Extract the pixel values form the landsat raster at the locations in the training data 
...
# Inspect level to understand order of classes in prediction 
...
dt_plot <- tm_shape(SB_classification_dt) + 
  tm_raster(
    col.scale = tm_scale(
      values = c("#8DB590", "#F2DD84", "#7E8987", "#6A8EAE"), 
      labels = c("green vegetation", "soil/dead grass", "urban", "water")
    ),
    col.legend = tm_legend("Landcover Types")
  ) + 
...
# Take the XY coordinate from landsat dataframe and convert them to cells numbers for the raster
...
kmeans_plot <- tm_shape(SB_classification_kmeans) + 
  tm_raster(
    col.scale = tm_scale(
      values = c("#F2DD84", "#7E8987", "#8DB590", "#6A8EAE"), 
