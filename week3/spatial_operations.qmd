...
# filter to the region of Cantebury 
canterbury <- nz %>% 
...
```{r}
#  subset nz heights to just the features that intersects Canterbury 
c_height1 <- nz_height[canterbury, ] # automatic intersect with brackets
```
...
```{r}
# subset to the features of Canterbury 
...
# Filter based on a logical vector 
...
```{r}
# find height withing 100 kilometers from Canterbury 
...
```{r}
# joined_df <- leftjoin(df_A, df_B, by = c("id_A" = "id_B"))
```
...
```{r}
# check whether or not the points overlap 
...
# Use area-weighted interpolation to aggregae the "value" attribute

aggregating_zones_area_weighted <- st_interpolate_aw(x = incongruent_2, aggregating_zones, 
                                                    extensive = TRUE)
...
# center used ot be red point now its blue point 
```
