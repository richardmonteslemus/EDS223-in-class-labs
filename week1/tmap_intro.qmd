...
tm_shape(world_elevation) + # Plotting global elevation
  tm_raster() +  # tm_raster for raster data 
tm_shape(world_vector) + 
  tm_borders() + 
tm_shape(world_cities) + 
  tm_dots() + 
  tm_text("name", size = 0.7, col = "black")
...
# This allows the extent to be based on world cities without me needing to put it first before raster data.
...
```{r}
tm_shape(world_elevation, bbox = world_cities) + # bounding box equals extent of world cities
  tm_raster() + 
tm_shape(world_vector) + 
  tm_borders()
...
tm_shape(world_vector) + 
  tm_polygons(fill = "life_expectancy", 
              ...
              fill.legend = tm_legend(title = "Life Expectancy in (years)", position = tm_pos_out("left")))
