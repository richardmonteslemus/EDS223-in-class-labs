...
map1 <- tm_shape(austria) + 
  tm_raster(col.legend = tm_legend("Elevation (m)",
col.scale = tm_scale_continuous(values = c(0, 1000, 2000, 3000))))+ 
...
```{r}
map1 <- tm_shape(zion_elevation) + 
  tm_raster(col.legend = tm_legend(title = "Elevation (m)")) + tm_title(text = "Original")
...
```{r}
# Create raster mask of the same resolution extent 
...
map2 <- tm_shape(masked1) + 
  tm_raster(col.legend = tm_legend(title = "Elevatio (m)")) + 
...
zion_elevation_disagg <- disagg(zion_elevation_coarse, fact = 5, method = "bilinear") # Bilinear means 4 closest cells and assigning weight
