...
dtm_post <- rast(here::here( "data", "Eaton_2", "dtm_eaton_epsg32611_geoid12b_1m_POSTFIRE.tif"))
dsm_post <- rast(here::here( "data", "Eaton_2", "dsm_eaton_epsg32611_geoid12b_1m_POSTFIRE.tif"))
```

```{r}
damage <- st_read(here::here("data","06061784-f911-4d0e-aca5-0768ac66aac5.gdb")) %>% st_transform(crs = crs(dsm_post))
```
...
quantiles <- global(diff_dsm_crop, fun = quantile, 
                    probs = c(.005, .995), na.rm = TRUE) # Remove extreme outliers
...
overlay <- height_shade(
  diff_mat, 
  texture = colorRampPalette(c("#8B0000", "#FF4500", "#FFFFFF", 
                  "#87CEEB", "#00008B"))(256), 
  range = c(-20, 20)
)
